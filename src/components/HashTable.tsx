...
export const HashTable: React.FC<HashTableProps> = ({
  ...
  return (
    <div className="w-full max-w-5xl mx-auto flex flex-col items-center gap-4">
      ...
      <div className="w-full bg-white dark:bg-[#0B1228] border-2 border-blue-100/90 dark:border-blue-900/40 rounded-2xl p-5 sm:p-7 overflow-hidden shadow-md shadow-blue-900/5 dark:shadow-[0_8px_30px_rgba(0,0,0,0.35)] transition-all">
        {/* Table Header Bar with Royal Blue Accents */}
        <div className="flex flex-wrap items-center justify-between pb-3.5 mb-4 border-b border-slate-100 dark:border-blue-900/20 gap-2 font-sans">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg bg-blue-50 dark:bg-blue-950/60 border border-blue-100 dark:border-blue-800/40 flex items-center justify-center text-blue-600 dark:text-blue-300 shadow-2xs">
              <Layers className="w-4 h-4" />
            </div>
            <h3 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider font-mono">
              Hash Table Array (Capacity = {tableSize})
            </h3>
            <span className="text-xs bg-blue-50/60 dark:bg-[#080D1F] px-2.5 py-0.5 rounded-lg border border-blue-100 dark:border-blue-900/30 text-blue-700 dark:text-blue-300 font-mono font-bold shadow-2xs">
          ...
          <div className="flex items-center gap-3 text-xs text-slate-600 dark:text-slate-400 font-mono">
            <span className="flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-slate-50 dark:bg-[#080D1F] border border-slate-200/80 dark:border-blue-900/25 shadow-2xs">
              <span className="w-2.5 h-2.5 bg-blue-600 dark:bg-blue-600 rounded-xs shadow-2xs" /> Occupied
            </span>
            <span className="flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-blue-50 dark:bg-blue-950/50 border border-blue-200/80 dark:border-blue-800/40 text-blue-700 dark:text-blue-300 font-semibold shadow-2xs">
              <span className="w-2.5 h-2.5 bg-blue-600 dark:bg-blue-400 rounded-xs shadow-2xs" /> Target / Probe
            </span>
            {isChaining && (
              <span className="flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-slate-50 dark:bg-blue-950/40 border border-slate-200 dark:border-blue-900/30 text-slate-700 dark:text-blue-300 font-semibold shadow-2xs">
        ...
        {isChaining && (
          <div className="mt-4 pt-4 border-t border-slate-100 dark:border-blue-900/20 font-sans">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {slots
                ...
                .map((s) => (
                  ...
                  >
                    <span className="font-bold text-white bg-blue-600 dark:bg-[#0F1733] dark:border dark:border-blue-900/30 px-2 py-0.5 rounded-md shrink-0 font-mono shadow-xs">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0 font-mono">
                      {s.items.map((item, idx) => (
                        <React.Fragment key={item.id || idx}>
                          {idx > 0 && <ArrowRight className="w-3 h-3 text-blue-600 dark:text-blue-400 shrink-0" />}
